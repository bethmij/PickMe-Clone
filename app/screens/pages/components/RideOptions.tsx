...
const RideOptions = () => {
  ...
  return (
    <SafeAreaView>
        <View>
        <TouchableOpacity className='absolute top-3 left-5 z-50 p-3 rounded-full'
            onPress={()=>{
            ...
            renderItem={({item : {id, title, multiplier, image},item}) => (
                <TouchableOpacity 
                    className={`flex flex-row justify-between items-center px-4 py-2 rounded-lg ${
                        selected === item ? 'bg-gray-300' : 'bg-transparent'
                    }`}
        ...
        <View className='mt-auto border-t border-gray-200'>
            <TouchableOpacity className={`bg-black py-3 m-3 ${!selected && 'opacity-50'}`} disabled={!selected}>
